...
import { Package, Tag, DollarSign, Star, Box } from "lucide-react";
                ...
                <table className="w-full text-left border-collapse min-w-175">
                    <thead>
                        <tr className="bg-slate-100/50 text-slate-500 text-xs uppercase tracking-wider border-b border-slate-200">
                            ...
                            <th className="p-4 font-semibold whitespace-nowrap"><DollarSign className="w-3.5 h-3.5 inline mr-1.5" /> Price</th>
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {products.map((p) => (
                            <tr key={p.id} className="glass-panel-hover hover:bg-white/50 cursor-pointer" onClick={() => router.push(`/products/${p.id}`)}>
                                ...
                                <td className="p-4 text-slate-900 font-medium">${p.price.toFixed(2)}</td>
